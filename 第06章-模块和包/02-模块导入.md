...
def import_module(module_name):
    ...
    if module_name in sys.modules:
        print(f"从缓存中找到 {module_name}")
        return sys.modules[module_name]

    print(f"缓存中没有 {module_name}，开始查找...")
    # 继续后续步骤...
```

**关键点**：
- 这是import开始时做的第一件事
- 如果模块已在缓存中，直接返回，不会重复执行模块代码
- 这就是为什么模块是单例的
...
def find_module(module_name):
    ...
    if module_name in sys.builtin_module_names:
        ...
        return create_builtin_module(module_name)
    ...
    for path in sys.path:
        ...
        if os.path.exists(py_file):
            print(f"找到源文件: {py_file}")
            return py_file

        # 查找 module_name.pyc
        pyc_file = os.path.join(path, f"{module_name}.pyc")
        if os.path.exists(pyc_file):
            print(f"找到字节码文件: {pyc_file}")
            return pyc_file
        ...
        if os.path.exists(init_file):
            print(f"找到包: {package_dir}")
            return package_dir

    raise ImportError(f"No module named '{module_name}'")
```

#### 第三步：加载和执行（Loading & Execution）
```python
def load_and_execute_module(module_name, module_source):
    """加载和执行模块的伪代码"""
    ...
    try:
        ...
        print(f"执行失败，从缓存中移除 {module_name}")
        raise e
```
...
#### 第四步：绑定名称（Binding）
```python
def bind_module(module, import_statement):
    """绑定模块到本地作用域的伪代码"""
    ...
    if import_statement.startswith("import "):
        ...
        if " as " in import_statement:
            _, alias = import_statement.split(" as ")
            locals()[alias.strip()] = module
        else:
        ...
        attr_name = parts[3]  # some_function

        if len(parts) > 4 and parts[4] == "as":
            alias = parts[5]
            locals()[alias] = getattr(module, attr_name)
        else:
...
# module_b.py
import module_a  # 导入module_a - 这会造成循环导入！
